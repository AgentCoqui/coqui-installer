...
.DESCRIPTION
    ...
    This path is unsupported and retained only for at-risk users.
...
function Write-WindowsSupportWarning {
    Write-Warn "Native Windows install is degraded and not a supported target."
    Write-Host "  This path works as a byproduct of development and basic cross-platform coverage."
    Write-Host "  Use WSL2 or Docker on Windows for the best Coqui experience."
    Write-Host ""
}

function Confirm-NativeInstallRisk {
    if ($env:COQUI_NATIVE_INSTALL_ACKNOWLEDGED -eq '1') {
        return
    }

    Write-Warn "You are about to continue with the unsupported native Windows installer."
    Write-Host "  This path is not recommended and may fail in ways the project does not support."
    Write-Host "  Supported Windows installs use WSL2. Docker is the fallback alternative."
    Write-Host ""

    try {
        $reply = Read-Host "  Type NATIVE to continue"
    } catch {
        Write-Fatal "Native Windows install requires explicit confirmation. Re-run interactively or set COQUI_NATIVE_INSTALL_ACKNOWLEDGED=1 for an at-risk non-interactive run."
    }

    if ($reply -cne 'NATIVE') {
        Write-Fatal "Native Windows installation cancelled. Use the supported WSL2 installer instead."
    }
...
function Show-Usage {
    Write-Host "Usage: .\install-native.ps1 [flags]"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "Examples:"
    Write-Host "  .\install-native.ps1         # Install latest release"
    Write-Host "  .\install-native.ps1 -Dev    # Install from git (development)"
    Write-Host "  .\install-native.ps1 -Quiet  # Install with minimal output"
    Write-Host ""
    Write-Host "To bypass the confirmation prompt in automation, set:"
    Write-Host "  `$env:COQUI_NATIVE_INSTALL_ACKNOWLEDGED = '1'"
}
...
function Main {
    ...
    Show-Banner
    Write-WindowsSupportWarning
    Confirm-NativeInstallRisk
