...
function Show-Usage {
    Write-Host "Usage: .\uninstall-native.ps1 [flags]"
    Write-Host ""
    Write-Host "Removes the unsupported native Windows Coqui installation."
    Write-Host ""
    Write-Host "Supported Windows installations use WSL2. This native path is retained at your own risk."
    Write-Host ""
    Write-Host "Flags:"
    ...
    Write-Host ""
    Write-Host "Examples:"
    Write-Host "  .\uninstall-native.ps1                      # Interactive uninstall (workspace preserved)"
    Write-Host "  .\uninstall-native.ps1 -RemoveWorkspace     # Remove workspace data too"
    Write-Host "  .\uninstall-native.ps1 -Force               # No prompts, preserve workspace"
    Write-Host "  .\uninstall-native.ps1 -Force -All          # No prompts, remove everything"
}
...
function Show-Banner {
    ...
    Write-Host -Object "  ·▀▀▀  ▀█▄▀▪·▀▀█.  ▀▀▀ ▀▀▀·▀▀▀▀  ▀█▄▀▪ ▀▀▀ " -ForegroundColor Green
    Write-Host ""
    Write-Host "  Coqui Native Uninstaller (Unsupported Windows Path)"
    Write-Host ""
}
...
function Main {
    ...
    if (-not (Test-Path $COQUI_INSTALL_DIR)) {
        ...
        Write-Host "    `$env:COQUI_INSTALL_DIR = 'C:\path\to\coqui'; .\uninstall-native.ps1"
        Write-Host ""
        return
    }
